...
if (isset($_POST["update-contact"])) {
  ...
  $query .= "contact_mobile='$mobile' WHERE id=1";
...
if (isset($_POST["update-resume"])) {
  // print_r($_FILES);
  $imagename = time() . $_FILES["resumeFile"]["name"];
  $imgtemp = $_FILES["resumeFile"]["tmp_name"];

  if ($imgtemp == "") {
    $q = "SELECT * FROM cv WHERE 1";
    $r = mysqli_query($db, $q);
    $d = mysqli_fetch_array($r);
    $imagename = $d["cv_file"];
  }

  move_uploaded_file($imgtemp, "../cv/$imagename");

  $query = "UPDATE cv SET ";

  $query .= "cv_file='$imagename' WHERE id=1";

  $run = mysqli_query($db, $query);
  if ($run) {
    echo "<script>window.location.href='../admin/index.php?uploadresume=true';</script>";
  }
}
...
if (isset($_POST["add-resume"])) {
  ...
  try {
    ...
    $about = $_POST["about"];
    $org = $_POST["org"];
    $official_website_link = $_POST["official_website_link"];

    $query = "INSERT INTO resume (resume_type, title, resume_time, org, about_exp,official_website_link) VALUES (?, ?, ?, ?, ?, ?)";
    $stmt = mysqli_prepare($db, $query);

    if (!$stmt) {
      echo "Preparation failed: " . mysqli_error($db);
    } else {
      mysqli_stmt_bind_param($stmt, "ssssss", $type, $title, $time, $org, $about, $official_website_link);
